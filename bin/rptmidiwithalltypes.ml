...
module PORTMIDI_SERIALIZER = struct
  let serialize (message : MIDI_MESSAGE.t) : Bytes.t =
    let status_byte = (
      ...
      | ActiveSensing -> 0xFE
      | SystemReset -> 0xFF
      ) lor message.channel in

    let data1_byte = Char.chr message.data1 in
    let data2_byte = Char.chr message.data2 in
    let bytes =
      match message.message_type with
      | NoteOff -> Bytes.of_string (Printf.sprintf "%c%c%c" (char_of_int status_byte) data1_byte data2_byte)
      | NoteOn -> Bytes.of_string (Printf.sprintf "%c%c%c" (char_of_int status_byte) data1_byte data2_byte)
      | PolyphonicKeyPressure -> Bytes.of_string (Printf.sprintf "%c%c%c" (char_of_int status_byte) data1_byte data2_byte)
      | ControlChange -> Bytes.of_string (Printf.sprintf "%c%c%c" (char_of_int status_byte) data1_byte data2_byte)
      | ProgramChange -> Bytes.of_string (Printf.sprintf "%c%c" (char_of_int status_byte) data1_byte)
      | ChannelPressure -> Bytes.of_string (Printf.sprintf "%c%c" (char_of_int status_byte) data1_byte)
      | PitchBend -> Bytes.of_string (Printf.sprintf "%c%c%c" (char_of_int status_byte) data1_byte data2_byte)
...
let midi_to_bytes (message : MIDI_MESSAGE.t) : Bytes.t =
  ...
  module MIDI_MESSAGE = struct
    ...
    let serialize (message : t) : Bytes.t =
      ...
      let bytes =
        match message.message_type with
        | NOTE_OFF -> Bytes.of_string (Printf.sprintf "%c%c%c" (char_of_int (0x80 lor message.channel)) data1_byte data2_byte)
        | NOTE_ON -> Bytes.of_string (Printf.sprintf "%c%c%c" (char_of_int (0x90 lor message.channel)) data1_byte data2_byte)
        | POLY_PRESSURE -> Bytes.of_string (Printf.sprintf "%c%c%c" (char_of_int (0xA0 lor message.channel)) data1_byte data2_byte)
        | CONTROL_CHANGE -> Bytes.of_string (Printf.sprintf "%c%c%c" (char_of_int (0xB0 lor message.channel)) data1_byte data2_byte)
        | PROGRAM_CHANGE -> Bytes.of_string (Printf.sprintf "%c%c" (char_of_int (0xC0 lor message.channel)) data1_byte)
        | CHANNEL_PRESSURE -> Bytes.of_string (Printf.sprintf "%c%c" (char_of_int (0xD0 lor message.channel)) data1_byte)
        | PITCH_BEND -> Bytes.of_string (Printf.sprintf "%c%c%c" (char_of_int (0xE0 lor message.channel)) data1_byte data2_byte)
    ...
    let deserialize (bytes : Bytes.t) : t =
      ...
      let message_type, data1, data2 =
        if length = 2 then
          ...
          let data2 =
            if message_type = PROGRAM_CHANGE || message_type = CHANNEL_PRESSURE then 0
            else Bytes.get bytes 2
          in
          (message_type, data1, data2)
      in
      let timestamp = 0 in (* Set timestamp to 0 for now *)
      let message = create ~message_type ~channel ~data1:(int_of_char data1) ~data2:(int_of_char data2) ~timestamp in
